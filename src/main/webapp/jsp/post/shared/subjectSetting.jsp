<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
                ...
                <div class="card-header border-0 pt-6">
                    ...
                    <div class="card-toolbar">
                        ...
                        <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                            <!--begin::Add user-->
                            <form action="/subject/setting/add">
                        ...
                        <tbody class="text-gray-700 fw-bold">
                            ...
                            <c:forEach items="${requestScope.ssList}" var="ss">
                                <tr>
                                    <td></td>
                                    <td></td>
                                    <td><a onclick="details(${ss.settingId})"><b
                                                class="text-gray-600">${ss.title}</b></a></td>
                                    <td><b class="text-gray-600">${ss.subjectName}</b></td>
                                    <td>${ss.typeTitle}</td>
                                    <td>${ss.value}</td>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
                                                <c:if test="${ss.statusId==1}">
                                                    <li>
                                                        <button
                                                            onclick="changeStatus('${ss.settingId}', '${ss.title}','${ss.subjectName}', 'deactivate')"
                                                ...
                                                <c:if test="${ss.statusId == 0}">
                                                    <li>
                                                        <button
                                                            onclick="changeStatus('${ss.settingId}', '${ss.title}','${ss.subjectName}', 'activate')"
                                                            type="button" class="dropdown-item" data-bs-toggle="modal"
                                                            data-bs-target="#exampleModal">
                                                            Activate
                                                        </button>
                                                ...
                                                <li><a class="dropdown-item"
                                                        onclick="details(${ss.settingId})">Details</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                    <!--end::Action=-->
    ...
    <form hidden id="form1" action="/subject/setting?action=list" method="post">
        <input id="subjectId" name="subjectFilter" />
        <input id="typeId" name="typeFilter" />
    ...
    <form id="form2" action="" method="post">
        <input hidden id="settingId" name="settingId" />
    </form>
    ...
    <script type="text/javascript">
        function list(page) {
            ...
            document.getElementById('typeId').value = document.getElementById('typeFilter').value;
        ...
        function changeStatus(settingId, settingTitle, subjectName, action) {
            document.getElementById('content_modal').innerHTML =
                'Do you want to ' + action + " " + settingTitle + " of" + subjectName + "?";
            document.getElementById('settingId').value = settingId;
            document.getElementById('form2').action = '/subject/setting?action=changeStatus';
        }

        function details(settingId) {
            document.getElementById('settingId').value = settingId;
            document.getElementById('form2').action = '/subject/setting?action=get';
