...
public class SubjectSettingServiceImpl implements SubjectSettingService {
    ...
    public List<SubjectSetting> getSubjectSettingList(int start, int length,
            String search, String subjectFilter, String typeFilter,
            String displayOrderFilter, String statusFilter, String sort) {
        Connection connection = null;
        List<SubjectSetting> list = null;
        try {
            connection = BaseDao.getConnection();
            list = subjectSettingDao.getSubjectSettingList(connection, start, length, search, subjectFilter, typeFilter, displayOrderFilter, statusFilter, sort);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    @Override
    public int countAll(String search, String subjectFilter, String typeFilter, String displayOrderFilter, String statusFilter) {
        Connection connection = null;
        int count = 0;
        try {
            connection = BaseDao.getConnection();
            count = subjectSettingDao.countAll(connection, search, subjectFilter, typeFilter, displayOrderFilter, statusFilter);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            BaseDao.closeResource(connection, null, null);
