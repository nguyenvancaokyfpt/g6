...
public class MilestoneServiceImpl implements MilestoneService {
    ...
    public int countAll() {
       Connection connection = null;
        ...
        return count; }
    @Override
    public boolean add(MilestoneRequest milestoneRequest) {
        Connection connection = null;
        boolean flag = false;
        try {
            connection = BaseDao.getConnection();
            if (mileStoneDao.add(connection, milestoneRequest) > 0) {
                flag = true;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @Override
    public boolean update(Milestone milestone, int assId, int classId, Date fromDate, Date toDate, String title, String assBody, String desscription, int statusId) {
        Connection connection = null;
        try {
            connection = BaseDao.getConnection();
            mileStoneDao.update(connection, milestone.getMilestoneId(), assId, classId, fromDate ,toDate ,title , assBody,desscription , statusId);
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
