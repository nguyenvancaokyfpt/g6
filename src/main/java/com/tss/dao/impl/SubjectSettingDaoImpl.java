...
public class SubjectSettingDaoImpl implements SubjectSettingDao {
    ...
    public List<SubjectSetting> getSubjectSettingList(Connection connection, int start, int length, String search,
            String subjectFilter, String typeFilter, String displayOrderFilter, String statusFilter, String sort) throws SQLException {
        PreparedStatement preparedStatement = null;
        ResultSet resultSet = null;
        List<SubjectSetting> list = new ArrayList<>();
        if (connection != null) {
            String sql = "SELECT ss.*, s.subject_name\n"
                    + "FROM subject_setting ss join subject s on ss.subject_id = s.subject_id\n"
                    + "WHERE (s.subject_name LIKE ? OR ss.setting_title LIKE ?)\n"
                    + "AND ss.subject_id like ? and type_id LIKE ? AND setting_value LIKE ? AND ss.status_id LIKE ?\n"
                    + "LIMIT ?,?";
            if (!sort.equals("")) {
                sql = "SELECT ss.*, s.subject_name\n"
                        ...
                        + "WHERE (s.subject_name LIKE ? OR ss.setting_title LIKE ?)\n"
                        + "AND ss.subject_id like ? and type_id LIKE ? AND setting_value LIKE ? AND ss.status_id LIKE ?\n"
                        + "order by setting_value " + sort + " LIMIT ?,?";
            }
            Object[] params = {"%" + search + "%", "%" + search + "%", "%" + subjectFilter + "%", "%" + typeFilter + "%",
            ...
            try {
                ...
                while (resultSet.next()) {
                    ...
                    subjectSetting.setValue(resultSet.getString("setting_value"));
    ...
    @Override
    public int countAll(Connection connection, String search, String subjectFilter, String typeFilter, String displayOrderFilter,
